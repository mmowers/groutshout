 ...
 */
(function ($) {
  Drupal.behaviors.newpost = { 
    attach: function(context,settings) {
      $('#edit-field-body-und-0-value').focus();
      
      //$('#edit-submit').before('<input type="submit" id="pre-post-submit">Share!</input>');
      //$('#edit-submit').hide();
      $('#edit-submit').attr('value','Shout!');
      $('#edit-submit').click(function(){
        geolocate();
        return false;
      });
      //The following body modifications should be moved to php, but drupal was being annoying
      $('#edit-field-body-und-0-value').attr('maxlength','300');
      $('#edit-field-body-und-0-value').attr('placeholder',
        "Post a helpful message about your current location! (300 character limit)");


      $('#edit-submit').after('<div id="loader" style="width:23px;height:23px">');
      /**
      ...
      function geolocate(){
        ...
        if( navigator.geolocation ){
          ...
          // Call getCurrentPosition or watchPosition with success and failure callbacks. I'm doing something wrong with watchPosition so using getCurrentPosition for now
          navigator.geolocation.getCurrentPosition( success, fail, { enableHighAccuracy: true, timeout: timeoutVal, maximumAge: 0 });
          //var watchID = navigator.geolocation.watchPosition( success, fail, { enableHighAccuracy: true, timeout: timeoutVal, maximumAge: 0 });
        }
        else{
      ...
      function fail(error){
        ...
        var errors = { 
          ...
          //3: inaccurate_msg_start+accuracy_feet_rounded+inaccurated_msg_end
        };
